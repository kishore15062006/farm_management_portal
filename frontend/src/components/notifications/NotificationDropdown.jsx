    ...
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="relative p-2">
          ...
          {unreadCount > 0 && (<Badge className="absolute -top-1 -right-1 px-1.5 py-0.5 text-xs" variant="destructive">
              {unreadCount}
            </Badge>)}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-80 opacity-100 bg-popover notification-dropdown-content">
        <div className="p-3 border-b">
          <div className="flex items-center justify-between">
            ...
            {notifications.length > 0 && (<Button size="sm" variant="ghost" className="text-xs" onClick={markAllAsRead}>
                Mark all read
              </Button>)}
          </div>
        </div>
        ...
        {notifications.length === 0 ? (<div className="p-4 text-center text-muted-foreground">
            ...
            {notifications.slice(0, 5).map((notification) => (<DropdownMenuItem key={notification.id} className={`p-3 cursor-pointer ${!notification.read ? 'bg-accent/50' : ''}`} onClick={() => markAsRead(notification.id)}>
