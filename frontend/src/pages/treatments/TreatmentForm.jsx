...
const TreatmentForm = () => {
    ...
    const calculateWithdrawalDate = (drug, startDate, duration) => {
        const drugInfo = drugDatabase[drug.toLowerCase()];
    ...
    return (<div className="max-w-4xl mx-auto space-y-6 animation-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="animalId" value={formData.animalId} onChange={(e) => setFormData(prev => ({ ...prev, animalId: e.target.value }))} placeholder="e.g., #247, Cow-001" required/>
              </div>
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                {formData.drug && (<Badge className={getRiskColor(getDrugRiskLevel(formData.drug))}>
                    {getDrugRiskLevel(formData.drug)} risk
                  </Badge>)}
              </div>
              <div className="space-y-2">
                <Label htmlFor="dosage">Dosage</Label>
                <Input id="dosage" value={formData.dosage} onChange={(e) => setFormData(prev => ({ ...prev, dosage: e.target.value }))} placeholder="e.g., 500mg, 2ml/kg" required/>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="duration" type="number" value={formData.duration} onChange={(e) => setFormData(prev => ({ ...prev, duration: e.target.value }))} placeholder="7" required/>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-muted rounded-md flex items-center">
                  {withdrawalDate ? (<span className="text-foreground">{format(withdrawalDate, "PPP")}</span>) : (<span className="text-muted-foreground">Auto-calculated after input</span>)}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea id="reason" value={formData.reason} onChange={(e) => setFormData(prev => ({ ...prev, reason: e.target.value }))} placeholder="Describe the condition or symptoms being treated..." required/>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="vetPrescription" value={formData.vetPrescription} onChange={(e) => setFormData(prev => ({ ...prev, vetPrescription: e.target.value }))} placeholder="VET-2024-001" className="bg-card text-foreground border-border placeholder:text-muted-foreground"/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="batchNumber">Drug Batch Number</Label>
                <Input id="batchNumber" value={formData.batchNumber} onChange={(e) => setFormData(prev => ({ ...prev, batchNumber: e.target.value }))} placeholder="BT-240115-AM" required/>
              </div>
            </div>

            {/* Withdrawal Warning */}
            {formData.drug && getDrugRiskLevel(formData.drug) === 'high' && (<div className="flex items-start space-x-3 p-4 bg-warning/10 border border-warning/20 rounded-lg">
