...
const RegulatorDashboard = () => {
    ...
    const activeTreatments = treatments.filter(t => (t.status === 'approved' && new Date(t.withdrawalDate) > now) || t.status === 'pending');
    ...
    const complianceViolations = (notifications.filter(n => n.type === 'compliance_alert').length +
      ...
      <Card className="card-elevated">
        ...
        <CardContent>
          <div className="space-y-4">
            {relevantAlerts.length === 0 ? (<div className="text-center py-6 text-muted-foreground">{t('regDashboard.noRecentAlerts')}</div>) : (relevantAlerts.map((n) => {
          ...
          return (<div key={n.id} className="flex items-start space-x-3 p-3 rounded-lg border border-border">
                    {getAlertIcon(n.type)}
